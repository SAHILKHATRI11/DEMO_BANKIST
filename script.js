...
const formatMovementDate = function (date) {
  ...
  const displayDate = calcDaysPassed(new Date(), date);
  if (displayDate === 0) return 'Today';
  else if (displayDate === 1) return 'Yesterday';
  else if (displayDate <= 7) return `${displayDate} days ago`;
  else {
    ...
    const month = `${date.getMonth() + 1}`.padStart(2, 0); //because javaScript gives month zero indexed.
